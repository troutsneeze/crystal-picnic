...
bool Configuration::load(void)
{
	...
	const char *v_purchased = al_get_config_value(acfg, "miscellaneous", "purchased");
	if (v_purchased) {
		engine->set_purchased(!strcmp(v_purchased, "true"));
		LOG("Read purchased: " + std::string(v_purchased));
	}
	else {
		LOG("Warning: purchased not present.");
	}
#endif
...
#if defined ALLEGRO_IPHONE && defined ADMOB
	al_set_config_value(acfg, "miscellaneous", "purchased", engine->get_purchased() ? "true" : "false");
#endif
