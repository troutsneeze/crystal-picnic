...
ALLEGRO_TRANSFORM korean_transform;
...
{
	return al_get_text_width(font, text.c_str())/font_scale;
}

int get_text_width(Font_Type font, std::string text)
{
	return al_get_text_width(get_font(font), text.c_str())/font_scale;
}

int get_font_line_height(Font_Type font)
{
	int h = al_get_font_line_height(get_font(font))/font_scale;
	if (cfg.language == "Korean") {
		h+=2; // NanumGothicBold reports slightly small line height, increase it
	}
	return h;
}
...
void load_fonts()
{
	...
	if (cfg.language == "Korean") {
		font_scale = cfg.screens_h;
		fonts[FONT_LIGHT] = General::load_font("fonts/NanumGothicBold.ttf", 8*font_scale);
		fonts[FONT_HEAVY] = General::load_font("fonts/NanumGothicBold.ttf", 8*font_scale);
	}
	else {
		font_scale = 1.0f;
	...
	al_identity_transform(&korean_transform);
...
bool set_clipping_rectangle(int x, int y, int width, int height, bool scale)
{
	if (scale) {
		x *= cfg.screens_w;
		y *= cfg.screens_h;
...
void draw_wrapped_text(std::string text, ALLEGRO_COLOR color, float x, float y, float width, Font_Type font)
{
	...
	for (i = 0; i < len;) {
		...
		if (c == ' ') {
			...
			int w = al_get_ustr_width(get_font(font), u2)/General::font_scale;
			al_ustr_free(u2);
			if (w > width) {
	...
	int w = al_get_ustr_width(get_font(font), u)/General::font_scale;
	if (w > width) {
		ALLEGRO_USTR *u3 = al_ustr_dup_substr(u, 0, last_good);
		Graphics::draw_ustr(
			get_font(font),
...
#ifdef _WIN32
	if (cfg.fullscreen) {
		ShowWindow(al_get_win_window_handle(engine->get_display()), SW_MINIMIZE);
	}
...
#elif defined __linux__
	std::string argv0 = General::argv[0];
	size_t slash = argv0.rfind("/");
	std::string filename = argv0.substr(0, slash) + "/" + "stuff/3rd_party.html";
